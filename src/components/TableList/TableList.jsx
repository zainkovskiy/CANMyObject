      ...
      if (filter[key] !== 'all') {
        ...
        if (key === 'contract') {
          if (filter[key] === 'договор') {
            ...
            interimFilter = interimFilter.filter(
              (item) => item.contract.type === filter[key]
            );
          }
        }
        if (key === 'advStatus') {
          interimFilter = interimFilter.filter(
            (item) => item.advStatus === filter[key]
          );
        }
        if (key === 'assigned') {
          interimFilter = interimFilter.filter(
            (item) => item.assigned_by.fullName === filter[key]
          );
        }
        if (key === 'date') {
                    ...
                    <span
                      ...
                      onClick={() => {
                        BX.SidePanel.Instance.open(
                          `https://crm.centralnoe.ru/cardObject/?login=yes&source=1c&reqNumber=${row.reqNumber}`,
                          { animationDuration: 300, width: 980 }
                        );
                      }}
                    >
                  ...
                  <TableCell style={cell}>
                    ...
                    >
                      <div style={{ width: 'min-content' }}>
                        <span>{row?.contract?.typeName}</span>&nbsp;
                        {row?.contract?.dealId && (
                          <span
                            className='link-bitrix'
                            onClick={() => {
                              BX.SidePanel.Instance.open(
                                `https://crm.centralnoe.ru/crm/deal/details/${row.contract.dealId}/`,
                                { animationDuration: 300, width: 980 }
                              );
                            }}
                          >
